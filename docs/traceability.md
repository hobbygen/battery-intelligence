...
Status: updated after Phase 13. Version 1.0.0.
...
| R-099 | Performance budgets | §45 | all | Batched writes; 1 Hz UI coalescing (Phase 10); adaptive sampling (Phase 13, `AdaptiveSamplingPolicy`); `ISelfMetrics` + Diagnostics "footprint" section. Short in-session soak: CPU negligible, disk < 1 MB/min. **Working set ~230–270 MB > the 150 MB budget (WinUI 3 baseline)**; 24 h soak + frame profiling are Phase 14 | `AdaptiveSamplingPolicyTests`; `ResponsiveColumnsTests`; live soak | 🔨 (adaptive + self-metrics done; 24 h soak + working-set reduction Phase 14) |
